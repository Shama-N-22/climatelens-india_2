...
import {
  ArrowLeft, BarChart3, Droplets, Thermometer, Leaf, Building2, Waves,
  TrendingUp, TrendingDown, Minus, Sparkles, AlertTriangle, Search, Flame, X,
} from "lucide-react";
...
import { FeaturePanel, FeatureModal, type SelectedFeature } from "../components/dashboard/FeatureDetail";
import { DEFAULT_TIMELINE } from "../data/geeTimeline";
import MapView from "../components/map/MapView";

const PARAMETERS: { key: ParamKey; label: string; icon: typeof Droplets; blurb: string }[] = [
  { key: "lst", label: "Land Surface Temperature", icon: Thermometer, blurb: "Land surface temperature & urban heat islands." },
  { key: "ndvi", label: "Vegetation Index", icon: Leaf, blurb: "Vegetation health and canopy density." },
  { key: "ndwi", label: "Water Index", icon: Waves, blurb: "Surface water and moisture detection." },
  { key: "ndbi", label: "Built-up Index", icon: Building2, blurb: "Built-up area and urban expansion." },
];

const STATUS_STYLES: Record<Status, { dot: string; text: string; ring: string }> = {
  good: { dot: "bg-emerald-400", text: "text-emerald-300", ring: "ring-emerald-400/20" },
  watch: { dot: "bg-sky-400", text: "text-sky-300", ring: "ring-sky-400/20" },
  warn: { dot: "bg-amber-400", text: "text-amber-300", ring: "ring-amber-400/20" },
  critical: { dot: "bg-rose-500", text: "text-rose-300", ring: "ring-rose-500/20" },
};
...
function KpiCard({ kpi, index, onOpen }: { kpi: Kpi; index: number; onOpen: (k: Kpi) => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-1">
        <span style={{ fontFamily: "var(--font-mono)" }} className="truncate text-[9px] uppercase tracking-wider text-slate-400">
          {kpi.label}
        </span>
...
function KpiModal({ kpi, onClose }: { kpi: Kpi; onClose: () => void }) {
  const s = STATUS_STYLES[kpi.status];
  const compact = kpi.value >= 100000;
  const val = (compact ? kpi.value / 1000000 : kpi.value).toLocaleString("en-IN", {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          <span style={{ fontFamily: "var(--font-mono)" }} className="text-[10px] uppercase tracking-widest text-slate-400">
            {kpi.label}
          </span>
          <button onClick={onClose} className="text-slate-400 transition hover:text-slate-200">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className={`mt-2 flex items-center gap-1.5 text-sm ${s.text}`}>
          ...
          <span className="text-slate-500">
            {kpi.trend === "up" ? "vs last period ↑" : kpi.trend === "down" ? "vs last period ↓" : "steady"}
          </span>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-lg bg-white/5 px-3 py-2">
          ...
          <span className="text-xs text-slate-300">Status: {STATUS_LABEL[kpi.status]}</span>
        </div>
      </motion.div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const [featureModal, setFeatureModal] = useState<SelectedFeature | null>(null);
    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#0b1220] text-slate-200">
      ...
      <aside className="flex w-72 flex-col border-r border-white/10 bg-[#0d1526]/80">
        ...
        <div className="px-5 pb-3 pt-2">
          <p style={{ fontFamily: "var(--font-mono)" }} className="mb-2 text-[10px] uppercase tracking-widest text-slate-500">
            Cities
          </p>
          <div className="space-y-1">
            {CITIES.map((c) => (
              ...
              >
                <span>{c.name}</span>
                <span style={{ fontFamily: "var(--font-mono)" }} className="text-[10px] text-slate-500">
                  {c.state}
                </span>
              </button>
            ))}
        ...
        <div className="px-5 py-3">
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm transition ${
                    active ? "bg-teal-400/10 text-teal-200 ring-1 ring-teal-300/30" : "text-slate-300 hover:bg-white/5"
                  }`}
                >
          ...
          <p className="mt-3 px-1 text-xs leading-relaxed text-slate-500">{activeParam.blurb}</p>
        </div>
        ...
        <div className="px-5 py-3">
          ...
          <div className="space-y-1">
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm transition ${
                showUHI ? "bg-rose-500/15 text-rose-200 ring-1 ring-rose-400/30" : "text-slate-300 hover:bg-white/5"
              }`}
            >
              <Flame className="h-4 w-4 shrink-0" />
              UHI Dynamic Hotspot
              {showUHI && <span className="ml-auto text-[10px] font-semibold text-rose-300">ON</span>}
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto">
        ...
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-white/10 bg-[#0b1220] px-6 py-3.5">
          <div>
            <h1 style={{ fontFamily: "var(--font-display)" }} className="text-lg font-semibold text-slate-50">
              {city.name} <span className="text-slate-500">/ {activeParam.label}</span>
            </h1>
            <p style={{ fontFamily: "var(--font-mono)" }} className="text-[11px] text-slate-500">
              {city.center[0].toFixed(3)}°N, {city.center[1].toFixed(3)}°E · live
            </p>
          </div>
          <div className="flex items-center gap-3">
            <TimelineSelector year={year} month={month} onYear={setYear} onMonth={setMonth} />
            <button
              ...
              <div className="flex flex-col gap-4 pr-4">
                <div className="grid grid-cols-4 gap-3">
                  {city.kpis.slice(0, 8).map((k, i) => (
                    <KpiCard key={k.key + cityId} kpi={k} index={i} onOpen={setPopupKpi} />
                  ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <span style={{ fontFamily: "var(--font-display)" }} className="text-sm font-semibold text-slate-50">
                        AI Insight
                      </span>
                    ...
                    <div className="mt-3 rounded-lg border border-white/10 bg-[#0b1220]/60 p-3">
                      <p className="text-xs font-medium uppercase tracking-wider text-amber-300/80">Recommendation</p>
                      <p className="mt-1 text-sm text-slate-200">{insight.recommendation}</p>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                      {[
                        ...
                        <div key={x.l} className="rounded-lg bg-white/5 p-2">
                          <p style={{ fontFamily: "var(--font-mono)" }} className="text-[9px] uppercase tracking-wider text-slate-500">
                            {x.l}
                          </p>
                          <p className="mt-0.5 text-xs font-semibold text-slate-100">{x.v}</p>
                        </div>
                      ))}
                    </div>
                  </motion.div>
                ...
                <div className="rounded-2xl border border-white/10 bg-[#0f1a2e]/70 p-5">
                  ...
                  <div className="space-y-2.5">
                    {city.alerts.map((a) => {
                      ...
                      return (
                        <div key={a.id + cityId} className="rounded-lg border border-white/5 bg-[#0b1220]/50 p-3">
      ...
      {popupKpi && <KpiModal kpi={popupKpi} onClose={() => setPopupKpi(null)} />}
      {featureModal && <FeatureModal feature={featureModal} onClose={() => setFeatureModal(null)} />}
    </div>
  );
}