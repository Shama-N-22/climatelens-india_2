...
import { Layers, Building2, HeartPulse, Grid3x3, Droplets, ChevronDown, Check } from "lucide-react";
...
export default function LayersMenu({
  showBuildings, showHospitals, showWards,
  onToggleBuildings, onToggleHospitals, onToggleWards,
}: LayersMenuProps) {
  ...
  const items = [
    { icon: Building2, label: "Building footprints", on: showBuildings, toggle: onToggleBuildings },
    { icon: HeartPulse, label: "Hospitals", on: showHospitals, toggle: onToggleHospitals },
    { icon: Grid3x3, label: "Ward boundaries", on: showWards, toggle: onToggleWards },
  ];
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        Layers
        {activeCount > 0 && (
          <span className="rounded bg-teal-400/20 px-1.5 py-0.5 text-[10px] font-semibold text-teal-200">{activeCount}</span>
        )}
        ...
        <div className="absolute right-0 z-[1200] mt-2 w-60 rounded-xl border border-white/10 bg-[#0f1a2e] p-2 shadow-2xl">
          {items.map((it) => (
            ...
            >
              ...
              <span className={`grid h-4 w-4 place-items-center rounded border ${it.on ? "border-teal-400 bg-teal-400 text-slate-900" : "border-white/25"}`}>
          ...
          <div className="flex cursor-not-allowed items-center justify-between rounded-lg px-2.5 py-2 text-sm text-slate-500">
            <span className="flex items-center gap-2"><Droplets className="h-4 w-4" /> Public water</span>
