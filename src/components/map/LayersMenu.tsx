...
// "Layers" dropdown for map overlays. Buildings is live; the rest are shown
// as "soon" and will switch on when Prathyu's datasets arrive.
...
export default function LayersMenu({
  ...
  const activeCount = showBuildings ? 1 : 0;
  const soon = [
    { icon: HeartPulse, label: "Hospitals" },
    { icon: Droplets, label: "Public water" },
    { icon: Users, label: "Ward population" },
  ];
      ...
      {open && (
        <div className="absolute right-0 z-[1200] mt-2 w-60 rounded-xl border border-white/10 bg-[#0f1a2e] p-2 shadow-2xl">
          ...
          >
            ...
            >
              {showBuildings && <Check className="h-3 w-3" />}
            </span>
          </button>
          ...
          {soon.map((s) => (
            <div
              key={s.label}
              className="flex cursor-not-allowed items-center justify-between rounded-lg px-2.5 py-2 text-sm text-slate-500"
            >
              <span className="flex items-center gap-2">
