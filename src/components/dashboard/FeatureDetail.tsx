...
export function FeaturePanel({
  ...
  // Telangana's boundary layer is district-wise (p.district present);
  // Ahmedabad/Mumbai stay ward-wise (p.ward_no present) - same "ward" type,
  // just different underlying data, so labels switch based on what's there.
  const isDistrict = !isHospital && p.district != null;
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <h3 className="text-sm font-bold leading-tight text-slate-50">
              {p.name || p.district || (isHospital ? "Facility" : "Ward")}
            </h3>
          </div>
        </div>
        <button
...
export function FeatureModal({
  ...
  const p = feature.props || {};
  const isHospital = feature.type === "hospital";
  // Same district-vs-ward detection as FeaturePanel above.
  const isDistrict = !isHospital && p.district != null;
          ...
          {isHospital ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <Stat
                  icon={Grid3x3}
                  label={isDistrict ? "District name" : "Ward number"}
                  value={
                    isDistrict
                      ? (p.district ?? "—")
                      : (p.ward_no ?? p.name ?? "—")
